...
def _load_pp_model(cfg):
    if cfg.using_t5():  pp_model = AutoModelForSeq2SeqLM.from_pretrained(cfg.pp_name, local_files_only=False).to(cfg.device)
    else:               pp_model = AutoModelForSeq2SeqLM.from_pretrained(cfg.pp_name, local_files_only=False, max_position_embeddings = cfg.orig_max_length + 10).to(cfg.device)
    return pp_model

def _prepare_vm_tokenizer_and_model(cfg):
    vm_tokenizer = AutoTokenizer.from_pretrained(cfg.vm_name)
    vm_model = AutoModelForSequenceClassification.from_pretrained(cfg.vm_name, local_files_only=False).to(cfg.device)
    vm_model.eval()
...
def _prepare_nli_tokenizer_and_model(cfg):
    ...
    nli_model = AutoModelForSequenceClassification.from_pretrained(cfg.nli_name, local_files_only=False).to(cfg.device)
    nli_model.eval()
...
def _prepare_cola_tokenizer_and_model(cfg):
    ...
    cola_model = AutoModelForSequenceClassification.from_pretrained(cfg.cola_name, local_files_only=False).to(cfg.device)
    cola_model.eval()
